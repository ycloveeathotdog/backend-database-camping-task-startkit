...
select
    ...
    total_credit.total_purchased - used_credit.total_used as remaining_credit
from
(
        select
            user_id,
    ...
    inner join(
        ...
        from
            "COURSE_BOOKING"
        where
            status not in ('課程已取消', '即將授課')
        group by
            user_id
    ) used_credit on total_credit.user_id = used_credit.user_id
where
    total_credit.user_id = (
        select
            id
...
where
    cp.purchase_at >= '2024-12-01 00:00:00'
    and cp.purchase_at <= '2024-12-31 23:59:59'
group by
    cpk.name;
...
from
    "CREDIT_PURCHASE"
where
    purchase_at >= '2024-12-01 00:00:00'
    and purchase_at <= '2024-12-31 23:59:59';
